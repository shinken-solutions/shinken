...
class TestConfigNohgNosg(TestConfig):
    def setUp(self):
        self.setup_with_file('etc/nagios_1r_1h_1s_nohg_nosg.cfg')
        self.livestatus_broker = Livestatus_broker('livestatus', '127.0.0.1', str(50000 + os.getpid()), 'live', [],  'tmp/livelogs.db' + str(os.getpid()), 365, 'tmp/pnp4nagios_test' + str(os.getpid()), )
        self.livestatus_broker.properties = {
            'to_queue' : 0,
            'from_queue' : 0
    ...
    def test_thruk_log_current_groups(self):
        # From: Sven Nierlein
        # Subject: AW: AW: AW: AW: thruk / shinken
        # Merkwürdig, jetzt läufts durch. Mit und ohne Debug. Einzig die current_service_groups/current_host_groups kommen noch als "" an wenn keine Gruppen existieren.
        self.print_header() 
        now = time.time()
        host = self.sched.hosts.find_by_name("test_host_0")
        host.checks_in_progress = []
        ...
        self.show_logs()
        self.update_broker()
        self.scheduler_loop(1, [[host, 1, 'Down'], [router, 0, 'UP'], [svc, 0, 'OK']])
        self.show_logs()
        self.update_broker()
        cmd = "[%lu] SCHEDULE_SVC_DOWNTIME;test_host_0;test_ok_0;%d;%d;0;0;%d;lausser;blablub" % (now, now, now + 3600, 3600)
        self.sched.run_external_command(cmd)
        self.show_logs()
        self.update_broker()
        cmd = "[%lu] STOP_EXECUTING_HOST_CHECKS" % (now,)
        self.sched.run_external_command(cmd)
        self.show_logs()
        self.update_broker()
        cmd = "[%lu] START_EXECUTING_HOST_CHECKS" % (now,)
        self.sched.run_external_command(cmd)
        self.show_logs()
        self.update_broker()
        cmd = "[%lu] ENABLE_EVENT_HANDLERS" % (now,)
        self.sched.run_external_command(cmd)
...
Filter: time >= """ + str(int(now))
        ...
        request = """GET log
Columns: time current_service_groups current_host_groups
Filter: time >= """ + str(int(now))
        response, keepalive = self.livestatus_broker.livestatus.handle_request(request)
        print response
