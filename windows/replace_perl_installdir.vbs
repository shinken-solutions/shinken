...
Function GetShortPath(fullpath)
	...
	Dim tempfolder, tempfolderpath, Tempstring
	tempfolderpath = ""
	...
	' For perl progs, the \ are not very easy to process, so all the \ in the newstring will be replaced by /
	Tempstring = ""
	do
		returnvalue= ReplaceString("\\",returnvalue,"/")

		if TempString <> returnvalue then
			TempString = returnvalue
		else 
			Exit do
		end if
	loop while TempString = returnvalue
