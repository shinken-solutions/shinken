rem installation of shinken poller service using installutil....
...
echo Setting the shinken-poller.py from standard distro
copy /Y .\bin\shinken-poller .\bin\shinken-poller.py
echo Shinken Poller installed...
echo Updating the program path references
rem replacing the @@installdir@@ by %cd%
cscript replace_installdir.vbs shinken_poller.exe.config %tplpath% "%cd%"
cscript replace_installdir.vbs .\etc\pollerd-windows.ini %tplpath% "%cd%"
cscript replace_installdir.vbs .\bin\start_poller.cmd %tplpath% "%cd%"

echo installing the shinken poller service...
%iutil% Shinken_poller.exe
if "%errorlevel%"=="0" goto nextstep1
echo Error launched during installation of shinken poller.
