...
Publicly available services that are provided by Linux devices (like "HTTP", "FTP", "POP3", etc.) can be monitored easily by following the documentation on Monitoring :ref:`publicly available services (HTTP, FTP, SSH, etc.) <how-to-monitor/network-service>`.
...
To enable disk checking for the host, configure the :ref:`filesystem macro <advancedtopics/multi-layer-discovery#macros_mode>`:
